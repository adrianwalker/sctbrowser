SELECT COUNT (DISTINCT (
  ...
  d2.term))
FROM concept c1
INNER JOIN relationship r ON r.destination_id = c1.id
INNER JOIN concept c2 ON c2.id = r.source_id
...
WHERE c1.active = ANY(ARRAY[true, false])
AND c1.id = ?
AND r.active = true
AND c2.active = ANY(ARRAY[true, false])
AND d1.active = true
AND d1.type_id = ?
AND d2.active = true
AND d2.type_id = ?
